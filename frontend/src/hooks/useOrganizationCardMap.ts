...
export const useOrganizationCardMap = ({
  ...
  const reverseGeocode = useCallback(
    async (coords: MapCoords, ymaps: YMaps) => {
      setIsGeocoding(true);
      try {
        const response = await ymaps.geocode(
  ...
  const { isReady, setMarker, ymapsRef } = useYandexMap({
    ...
    onCoordsChange: (coords: MapCoords) => {
      setValueRef.current("latitude", coords.latitude, {
        shouldValidate: true,
      });
      setValueRef.current("longitude", coords.longitude, {
        shouldValidate: true,
      });
  ...
  const handleGetCurrentLocation = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        showErrorToast(
          messages[error.code] || "Не удалось получить местоположение",
        );
      },
