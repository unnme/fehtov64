...
async function generateClient() {
	try {
		...
		execSync('./node_modules/.bin/openapi-ts', {
			cwd: projectRoot,
			stdio: 'inherit'
		})

		// Post-process: add client export to index.ts
		const indexPath = join(projectRoot, 'src/client/index.ts')
		let indexContent = fs.readFileSync(indexPath, 'utf-8')
		if (!indexContent.includes("export { client }")) {
			// Add client export after the auto-generated comment
			const lines = indexContent.split('\n')
			const commentIndex = lines.findIndex(line => line.includes('auto-generated'))
			if (commentIndex !== -1) {
				lines.splice(commentIndex + 1, 0, '', "export { client } from './client.gen';")
				indexContent = lines.join('\n')
				fs.writeFileSync(indexPath, indexContent)
				console.log('✓ Added client export to index.ts')
			}
		}
