...
def read_persons(session: SessionDep, skip: int = 0, limit: int = 100) -> Any:
    ...
    # Load positions and build public data
    data = []
...
def create_person(*, session: SessionDep, person_in: PersonCreate) -> Any:
    ...
    if existing:
        raise HTTPException(
            status_code=409, detail="Person with this full name already exists"
        )
    ...
    if existing_email:
        raise HTTPException(status_code=409, detail="Почта уже используется")

    position = _get_position(session, person_in.position_id)  # Validate position exists
